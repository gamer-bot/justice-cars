...
import { X, Phone, Mail, MapPin, Gauge, Settings, Fuel, Heart, Flag, Share2, ChevronLeft, ChevronRight, MessageCircle } from "lucide-react";
...
export default function CarDetailModal({ car, onClose }: CarDetailModalProps) {
  ...
  async function submitInquiry() {
    ...
    try {
      const buyerName = userProfile.displayName || currentUser.email || "Anonymous";
      const buyerPhone = userProfile.phone || "";
  ...
  async function submitReport() {
    ...
    try {
      ...
      const msg = (err as { code?: string; message?: string })?.code === "permission-denied"
        ? "Reports are blocked by your Firestore security rules. Please allow signed-in users to write to the 'reports' collection."
        : `Could not submit the report: ${(err as Error).message || err}`;
      alert(msg);
    }
          ...
          <div className="grid md:grid-cols-2 gap-0">
            ...
            <div className="p-6 flex flex-col gap-4">
              <div>
                ...
                <h3 className="text-xl font-bold mt-1 leading-tight" style={{ fontFamily: 'Poppins, sans-serif' }}>
                  {car.title || [car.year, car.brand].filter(Boolean).join(" ")}
                </h3>
                {(car.year || car.brand) && (
                  <p className="text-sm text-muted-foreground mt-0.5">
                    {[car.year, car.brand].filter(Boolean).join(" • ")}
                  </p>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {!car.sold && (
                  <a
                    ...
                    className="flex-1 min-w-[140px] py-2.5 rounded-xl bg-[#25D366] text-white font-semibold text-sm hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
                  >
